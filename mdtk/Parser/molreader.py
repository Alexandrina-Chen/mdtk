...
# python 3.10.12
'''
Filename         : molreader.py
Description      : A class to read the mol file and store the data
Time             : 
Last Modified    : 
Author           : Sijia Chen
Version          : 1.0
...
class MolFrame(object):
    ...
    elements : np.array of str, the element name of atoms in this frame with shape (n_atoms, )
        Get the elements or indexes of atoms in this frame.
    positions : np.array, the positions of atoms in this frame with shape (n_atoms, # of dimensions)
        Get the positions of atoms in this frame. 
        The dimension of the positions is not necessarily 3
    ...
    def __init__(self, n_atoms: int, **kwargs) -> None:
        """
        Initialize the MolFrame object, which stores the information of one frame
...
class MolReader(object):
    """
    MolReader reads in the trajectory file and stores the information of all frames in the trajectory file.
    ...
    def __init__(self, filename, n_atoms, verbose = False,**kwargs) -> None:
    ...
    def _reopen(self):
        ...
        self.frame= self._frame(self._n_atoms, **self._frame_kwargs)
        self.frame.index = -1
    ...
    @functools.cached_property
    def n_frames(self):
        lines_per_frame = 1 + self._n_atoms
        counter = 0
        offsets = []
        with open(self.filename, 'r') as f:
            line = True
    ...
    def _read_next_frame(self) -> MolFrame:
        # initialize 
        ...
        frame.positions = atom_info[:,1:].astype(np.float64)

        return frame
        
        
    def rewind(self) -> MolFrame:
        self._reopen()
        self.next()

    def _read_frame(self, index) -> MolFrame:
        """
    ...
    def __getitem__(self, index) -> MolFrame:
...
def test(filename):
    reader = MolReader(filename,verbose=True)
    pass
